...
/*	Expand B to be a bitmap of BIT_CNT + SIZE bits
	and sets all of expanded bits to false.
	Returns true if success, false if memmory allocation
	failed. */
struct bitmap*
bitmap_expand(struct bitmap *bitmap, int size)
{
	...
	if(b != NULL)
	{
		b->bit_cnt = bitmap->bit_cnt + size;
		b->bits = malloc(byte_cnt(b->bit_cnt));
		if(b->bits != NULL || b->bit_cnt == 0)
		{
			bitmap_set_all(b,false);
			size_t i;
			for(i = 0 ; i<bitmap->bit_cnt;i++)
				bitmap_set(b,i,bitmap_test(bitmap,i));
